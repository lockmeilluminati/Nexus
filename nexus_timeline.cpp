...
void NexusTimeline::AddTrack(const std::string& name, int frames, bool isEnv) {
    ...
    tracks.push_back({name, 0, length, 0, frames, isEnv, false});
}
...
void NexusTimeline::Draw(int& globalFrame, float leftOffset, int& deleteIndex) {
    ...
    ImGui::SetNextWindowPos(
        ImVec2(viewport->WorkPos.x + leftOffset,
               viewport->WorkPos.y + viewport->WorkSize.y - timelineHeight),
        ImGuiCond_Always);
    ImGui::SetNextWindowSize(
        ImVec2(viewport->WorkSize.x - leftOffset, timelineHeight),
        ImGuiCond_Always);

    ImGui::Begin("Nexus Sequencer", nullptr,
        ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);

    // Transport controls
    if (ImGui::Button(isPlaying ? "PAUSE" : "PLAY", ImVec2(80, 25))) isPlaying = !isPlaying;
    ImGui::SameLine();
    if (ImGui::Button("STOP", ImVec2(60, 25))) { isPlaying = false; globalFrame = 0; }
    ImGui::SameLine();
    ...
    // Ruler
    draw_list->AddRectFilled(ImVec2(p.x, currentY),
                             ImVec2(p.x + width, currentY + 20),
                             IM_COL32(30, 30, 30, 255));
    for (int i = 0; i <= maxTimelineFrames; i += 60) {
        float tickX = p.x + headerWidth + (timelineWidth * ((float)i / maxTimelineFrames));
        draw_list->AddLine(ImVec2(tickX, currentY + 10), ImVec2(tickX, currentY + 20),
                           IM_COL32(150, 150, 150, 255));
        draw_list->AddText(ImVec2(tickX + 2, currentY), IM_COL32(150, 150, 150, 255),
                           (std::to_string(i / 60) + "s").c_str());
    }
    currentY += 25.0f;
    ...
    for (size_t i = 0; i < tracks.size(); i++) {
        ...
        // Row background
        draw_list->AddRectFilled(ImVec2(p.x, currentY),
                                 ImVec2(p.x + width, currentY + trackHeight - 4),
                                 IM_COL32(40, 40, 40, 255));

        // --- DELETE BUTTON (X) on the left ---
        ImGui::SetCursorScreenPos(ImVec2(p.x + 2, currentY + 6));
        ImGui::PushID((int)i * 10 + 99);
        ...
        // Track name label
        draw_list->AddText(ImVec2(p.x + 32, currentY + 10),
                           IM_COL32(200, 200, 200, 255),
                           track.assetName.c_str());
        ...
        draw_list->AddRectFilled(ImVec2(startX, currentY),
                                 ImVec2(endX, currentY + trackHeight - 4), clipColor, 4.0f);
        draw_list->AddRectFilled(ImVec2(startX, currentY),
                                 ImVec2(startX + 8, currentY + trackHeight - 4), handleColor, 4.0f);
        draw_list->AddRectFilled(ImVec2(endX - 8, currentY),
                                 ImVec2(endX, currentY + trackHeight - 4), handleColor, 4.0f);
        ...
        if (ImGui::IsItemActive() && ImGui::IsMouseDragging(ImGuiMouseButton_Left)) {
            float delta = (ImGui::GetIO().MouseDelta.x / timelineWidth) * maxTimelineFrames;
            track.startFrame += (int)delta;
        ...
        if (ImGui::IsItemActive() && ImGui::IsMouseDragging(ImGuiMouseButton_Left)) {
            ...
            track.startFrame += (int)delta;
            track.endFrame   += (int)delta;
            if (track.startFrame < 0) { track.startFrame = 0; track.endFrame = length; }
        }
    ...
    draw_list->AddLine(ImVec2(playheadX, p.y + 30),
